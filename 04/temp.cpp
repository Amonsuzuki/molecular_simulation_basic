...
double update_positions(vector<molecule>& fcc, double temp) {
	for (int i=0; i<N; i++) {
...
int main() {
	...
	vector<double> monte(STEP, 0), potential(STEP), temperature(13), ptemp(13);

	temperature[0] = 1;
	for (int i=0; i<13; i++) {
		if (i!=0)
			temperature[i] = temperature[i-1] +  0.25;
		potential[0] = initialize(fcc);
		for (int j=1; j<STEP; j++) {
			monte[j] = j;
			potential[j] = update_positions(fcc, temperature[i]);
		}
		double totalp = 0;
	...
	plt::named_plot("potential", temperature, ptemp);
	plt::xlabel("t");
	plt::ylabel("Energy");
