...
double initialize(vector<molecule>& fcc, vector<double>& kinetic, vector<r0>& initial_position, vector<v0>& initial_velocity, vector<double>& self_diffusion) {
...
double update_positions(vector<molecule>& fcc, vector<r0>& initial_position) {
	double total_diffusion = 0;
	for (int i=0; i<N; i++) {
...
double scaling_update_velocity(vector<molecule>& fcc, vector<double>& self_diffusion, vector<v0>& initial_velocity) {
	double ke = 0;
	for (int i=0; i<N; i++)
		ke += 0.5 * (pow(fcc[i].vx, 2) + pow(fcc[i].vy, 2) + pow(fcc[i].vz, 2));
	ke /= N;
...
double update_velocity(vector<molecule>& fcc, double& temp_equil, vector<double>& self_diffusion, vector<v0>& initial_velocity) {
	double ke = 0;
	for (int i=0; i<N; i++)
		ke += 0.5 * (pow(fcc[i].vx, 2) + pow(fcc[i].vy, 2) + pow(fcc[i].vz, 2));
	ke /= N;
...
int main() {
	...
	vector<double> t(STEP, 0), kinetic(STEP), potential(STEP), total(STEP), self_diffusion, self_diffusion2(STEP);
	double temp_equil = 0;
	double potential_equil = 0;
	vector<r0> initial_position(N);
	vector<v0> initial_velocity(N);

	potential[0] = initialize(fcc, kinetic, initial_position, initial_velocity, self_diffusion);
	total[0] = kinetic[0] + potential[0];

	for (int i=1; i<STEP; i++) {
		t[i] = i * Dt;
		potential[i] = update_positions(fcc, initial_position);
		if (i < STEP / 2)
			kinetic[i] = scaling_update_velocity(fcc, self_diffusion, initial_velocity);
		else {
			kinetic[i] = update_velocity(fcc, temp_equil, self_diffusion, initial_velocity);
