...
double calculate_force_and_potential(vector<molecule>& fcc, int sign, vector<double>& pressure, int index){
	double eps = 1.0;
	double sigma = 1.0;
	...
	if (index >= 0)
		pressure[index] = (1 / (3 * LX * LY * LZ)) * (pressure[index] + r_f);
	return(engp);
}

double initialize(vector<molecule>& fcc, vector<double>& kinetic, vector<r0>& initial_position, vector<v0>& initial_velocity, vector<double>& self_diffusion, vector<double>& pressure) {
	...
	for (int i=0; i<N; i++) {
		...
		initial_velocity[i].vx0 = fcc[i].vx;
		initial_velocity[i].vy0 = fcc[i].vy;
		initial_velocity[i].vz0 = fcc[i].vz;
		double v = fcc[i].vx * initial_velocity[i].vx0 + fcc[i].vy * initial_velocity[i].vy0 + fcc[i].vz * initial_velocity[i].vz0;
		total_diffusion += v;
	...
	pressure[0] = 2.0 * kinetic[0];
	return (calculate_force_and_potential(fcc, 0, pressure, 0));
}

double update_positions(vector<molecule>& fcc, vector<r0>& initial_position, vector<double>& pressure) {
	double total_diffusion = 0;
	for (int i=0; i<N; i++) {
	...
	//self_diffusion.push_back(total_diffusion / N);
	return (calculate_force_and_potential(fcc, 1, pressure, -1));
}

double scaling_update_velocity(vector<molecule>& fcc, vector<double>& self_diffusion, vector<v0>& initial_velocity, vector<double>& pressure, int index) {
	double ke = 0;
	for (int i=0; i<N; i++)
		ke += 0.5 * (pow(fcc[i].vx, 2) + pow(fcc[i].vy, 2) + pow(fcc[i].vz, 2));
	ke /= N;
	...
	self_diffusion.push_back(total_diffusion / N);
	calculate_force_and_potential(fcc, 1, pressure, index);
	return (engk);
}

double update_velocity(vector<molecule>& fcc, double& temp_equil, vector<double>& self_diffusion, vector<v0>& initial_velocity, vector<double>& pressure, int index) {
	double ke = 0;
	for (int i=0; i<N; i++)
		ke += 0.5 * (pow(fcc[i].vx, 2) + pow(fcc[i].vy, 2) + pow(fcc[i].vz, 2));
	ke /= N;
...
*/
void integrate_diffusion(vector<double>& self_diffusion, vector<double>& self_diffusion2) {
	self_diffusion2[0] = 0;
	for (int i=1; i<self_diffusion2.size(); i++)
		self_diffusion2[i] = self_diffusion2[i - 1] + 0.5 * (self_diffusion[i] + self_diffusion[i-1]) * Dt;
	cout << self_diffusion2.size() << endl;
}

int main() {
	vector<molecule> fcc(N);
	vector<double> t(STEP, 0), kinetic(STEP), potential(STEP), total(STEP), self_diffusion, self_diffusion2(STEP), pressure(STEP);
	double temp_equil = 0;
	double potential_equil = 0;
	vector<r0> initial_position(N);
	vector<v0> initial_velocity(N);

	potential[0] = initialize(fcc, kinetic, initial_position, initial_velocity, self_diffusion, pressure);
	total[0] = kinetic[0] + potential[0];

	for (int i=1; i<STEP; i++) {
		t[i] = i * Dt;
		potential[i] = update_positions(fcc, initial_position, pressure);
		if (i < STEP / 2)
			kinetic[i] = scaling_update_velocity(fcc, self_diffusion, initial_velocity, pressure, i);
		else {
			kinetic[i] = update_velocity(fcc, temp_equil, self_diffusion, initial_velocity, pressure, i);
			potential_equil += potential[i];
		}
		total[i] = kinetic[i] + potential[i];
		reset_forces(fcc);
	}
	integrate_diffusion(self_diffusion, self_diffusion2);
